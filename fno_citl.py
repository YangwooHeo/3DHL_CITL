...
SCRIPT_CONFIG = {
    ...
    "captured": None,  # Path(r"C:\...\aligned_camera.npy"), required for gradient_source="camera"
    "output_dir": Path("citl_axicon_fno_output"),

    # Optimization
    "gradient_source": "proxy",  # "proxy" or "camera"
    "iters": 1,
    "lr": 0.03,
    "optimizer": "adam",
...
def load_camera_image(
    ...
    arr = load_target_array(image_path)
    if transpose:
        arr = arr.T
    if flip_ud:
        arr = arr[::-1, :]
...
def camera_image_loss_gradient(
    ...
    captured_var = captured.detach().clone().requires_grad_(True)
    ...
    loss.backward()
    grad = captured_var.grad.detach().clone()
    loss_value = float(loss.detach().cpu().item())
    components["loss"] = loss_value
    return grad, loss_value, components
...
class AxiconFNOCITLSolver:
    ...
    def optimize(
        ...
        gradient_source = str(gradient_source).lower()
        if gradient_source not in ("proxy", "camera"):
            raise ValueError("gradient_source must be 'proxy' or 'camera'")
        if gradient_source == "camera" and captured is None:
            raise ValueError("gradient_source='camera' requires a homography-aligned captured camera image.")
        ...
        captured_grad = None
        captured_loss_value = None
        captured_components = None
        if gradient_source == "camera":
            captured = captured.detach().to(device=target.device, dtype=target.dtype)
            if captured.shape != target.shape:
                captured = resize_batch(captured, int(target.shape[-1]), mode="bilinear")
            captured_grad, captured_loss_value, captured_components = camera_image_loss_gradient(
                captured,
                target,
                loss_mode=loss_mode,
                roi_fraction=roi_fraction,
            ...
            if int(iters) > 1:
                print(
                    "[CITL] gradient_source='camera' is reusing one captured image for all iterations. "
                    "For physical iterative CITL, capture a fresh aligned camera frame after each phase update."
                )
        ...
        for idx in range(int(iters)):
            ...
            if gradient_source == "camera":
                components = dict(captured_components or {})
                loss_value = float(captured_loss_value or 0.0)
                pred.backward(gradient=captured_grad)
                if phase_tv_weight > 0:
                    tv_loss = float(phase_tv_weight) * wrapped_phase_tv(phase_var)
                    tv_loss.backward()
                    tv_value = float(tv_loss.detach().cpu().item())
                    loss_value += tv_value
                    components["phase_tv"] = tv_value
                components["loss"] = loss_value
            else:
                loss, components = camera_loss(
                    pred,
                ...
                if phase_tv_weight > 0:
                    tv_loss = float(phase_tv_weight) * wrapped_phase_tv(phase_var)
                    loss = loss + tv_loss
                    components["phase_tv"] = float(tv_loss.detach().cpu().item())
            ...
            if phase_var.grad is None:
                grad_last = torch.zeros_like(phase_var.detach())
            else:
...
def save_comparison_preview(
    ...
    save_path: Path,
    roi_fraction: float = 1.0,
    captured_camera: torch.Tensor | None = None,
):
    ...
    if captured_camera is not None:
        captured_view = apply_loss_roi(captured_camera, captured_camera, roi_fraction)[0][0, 0].detach().cpu().numpy()
    ...
    ncols = 5 if captured_view is not None else 4
    fig, axes = plt.subplots(3, ncols, figsize=(22 if ncols == 5 else 18, 12))
    axes = axes.reshape(3, ncols)
    axes[0, 0].imshow(target_view, cmap="gray", vmin=0, vmax=1)
    axes[0, 0].set_title("Target")
    pred_col = 1
    if captured_view is not None:
        axes[0, 1].imshow(captured_view, cmap="gray", vmin=0, vmax=1)
        axes[0, 1].set_title("Captured camera for dL/df")
        pred_col = 2
    axes[0, pred_col].imshow(np.clip(initial_pred_view / pred_scale, 0, 1), cmap="gray", vmin=0, vmax=1)
    axes[0, pred_col].set_title("Initial FNO camera")
    axes[0, pred_col + 1].imshow(np.clip(updated_pred_view / pred_scale, 0, 1), cmap="gray", vmin=0, vmax=1)
    axes[0, pred_col + 1].set_title("Updated FNO camera")
    axes[0, pred_col + 2].imshow(pred_delta, cmap="seismic", vmin=-pred_delta_scale, vmax=pred_delta_scale)
    axes[0, pred_col + 2].set_title("Updated - initial camera")
...
def write_run_config(path: Path, args: argparse.Namespace, fno_cfg: dict):
    ...
    serializable_args["captured"] = str(serializable_args.get("captured"))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--captured",
        type=Path,
        default=SCRIPT_CONFIG["captured"],
        help="Homography-aligned 1024x1024 camera PNG/NPY used when --gradient-source=camera.",
    )
    ...
    parser.add_argument(
        "--gradient-source",
        choices=("proxy", "camera"),
        default=SCRIPT_CONFIG["gradient_source"],
        help="'proxy' uses FNO output for dL/df; 'camera' uses captured image for dL/df and FNO only for df/dphase.",
    )
    parser.add_argument(
        "--loss-mode",
...
def main():
    ...
    args.gradient_source = str(args.gradient_source).lower()
    if args.gradient_source == "camera":
        args.captured = resolve_required_path(args.captured, "captured")
    elif args.captured is not None and str(args.captured).strip():
        args.captured = Path(args.captured).expanduser().resolve()
    else:
        args.captured = None
    ...
    if args.gradient_source == "camera":
        ...
        captured_metrics = compute_camera_metrics(
            captured,
            target,
            roi_fraction=roi_fraction,
        ...
        print(
            ...
            f"loss={captured_metrics['loss']:.6f}, "
            f"SSIM={captured_metrics['log_display_ssim']:.6f}, "
            f"mean_norm_mse={captured_metrics['mean_norm_mse']:.6f}"
        )
    ...
    captured_path = args.output_dir / f"{stem}_captured_camera_for_gradient.npy"
    ...
    if captured is not None:
        np.save(captured_path, captured[0, 0].detach().cpu().numpy().astype(np.float32))
    ...
    save_comparison_preview(
        ...
        roi_fraction=roi_fraction,
        captured_camera=captured,
    )
    extra_metrics = {}
    if captured_metrics is not None:
        extra_metrics["captured"] = captured_metrics
    save_metrics_report(
        metrics_json_path,
        metrics_csv_path,
        initial_metrics,
    ...
    if captured is not None:
        print(f"Saved captured camera used for gradient: {captured_path}")
    print(
