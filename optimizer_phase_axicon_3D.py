...
def _format_phase_for_slm(phase_slm, output_shape=(1200, 1600),
                          fill_mode='zero', random_seed=None):
    """Center a phase mask on the fixed-size SLM canvas.

    Parameters
    ----------
    phase_slm : ndarray [height, width]
        Integer phase mask in the existing SLM format (0--1023, int16).
    output_shape : tuple[int, int]
        SLM canvas shape as (height, width). The current SLM accepts
        1200 x 1600 arrays.
    fill_mode : {'zero', 'random'}
        ``zero`` fills unused pixels with phase level 0. ``random`` fills
        them with uniformly distributed phase levels from 0 through 1023.
    random_seed : int or None
        Optional seed for reproducible random padding. ``None`` produces a
        new random background on every call.
    """
    phase_slm = np.asarray(phase_slm)
    if phase_slm.ndim != 2:
        raise ValueError(f"phase_slm must be 2D, got shape {phase_slm.shape}.")

    output_h, output_w = map(int, output_shape)
    input_h, input_w = phase_slm.shape
    if output_h <= 0 or output_w <= 0:
        raise ValueError(f"output_shape must be positive, got {output_shape}.")
    if input_h > output_h or input_w > output_w:
        raise ValueError(
            f"Phase mask shape {phase_slm.shape} exceeds SLM output shape "
            f"{(output_h, output_w)}."
        )

    if fill_mode == 'zero':
        output = np.zeros((output_h, output_w), dtype=phase_slm.dtype)
    elif fill_mode == 'random':
        rng = np.random.default_rng(random_seed)
        output = rng.integers(
            0, 1024, size=(output_h, output_w), dtype=phase_slm.dtype)
    else:
        raise ValueError("fill_mode must be either 'zero' or 'random'.")

    offset_y = (output_h - input_h) // 2
    offset_x = (output_w - input_w) // 2
    output[offset_y:offset_y + input_h,
           offset_x:offset_x + input_w] = phase_slm
    return output
...
if __name__ == '__main__':
    ...
    slm_output_shape = (1200, 1600)  # (height, width) required by the SLM
    slm_fill_mode = 'zero'           # 'zero' | 'random'
    slm_random_seed = None            # Set an int for reproducible random fill
    ...
    beam_config.Nx_physical = 400
    beam_config.Ny_physical = 400
    ...
    for target_idx, target_path in enumerate(target_paths, start=1):
        ...
        phase_slm_output = _format_phase_for_slm(
            phase_slm,
            output_shape=slm_output_shape,
            fill_mode=slm_fill_mode,
            random_seed=slm_random_seed,
        )
        phase_output_path = output_directory / f"{target_path.stem}.npy"
        np.save(phase_output_path, phase_slm_output)
        ...
        print(
            f"Saved phase mask to {phase_output_path} "
            f"(shape={phase_slm_output.shape}, fill='{slm_fill_mode}')"
        )

        del optimized_phase, recon_vol, loss, phase_slm, phase_slm_output, rescaled_image
