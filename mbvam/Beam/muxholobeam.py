...
from mbvam.Beam.muxholobeamconfig import MuxHoloBeamConfig
from tqdm import tqdm

class MuxHoloBeam(HoloBeam):
    '''
    Instances of this class represent a time-mutliplexed holographic beam.
    ...
    Assumptions:
    - Each of the sub-beams are incoherent since they are separated in time.
    ...
    def __init__(self, beam_config): 
        ...
        if not isinstance(beam_config, MuxHoloBeamConfig):
            raise Exception("beam_config is not an instance of MuxHoloBeamConfig")
    ...
    def initializePhaseMaskAndEnergy(self, global_grid_energy_req, global_coord_vec):
        ...
        local_grid_energy_req = self.mapGlobalToLocal(global_grid_energy_req_per_sb, global_coord_vec, use_precomputed_sampling_coord=False)
    ...
    def propagateAllSubBeamsToGlobalVolumeIntensity(self, global_coord_vec):
        ...
        For performance improvement, future implementation should accumulate the sum in local_vol_intensity and only perform one mapLocalToGlobal.
        '''
        ...
        for sb_idx in range(self.beam_config.num_sub_beam):
            ...
            if self.beam_config.grad_checkpoint:
                ...
                global_vol_intensity += checkpoint(self.mapLocalToGlobal, local_vol_intensity,
                                                    global_coord_vec, 
                                                    use_precomputed_sampling_coord=False,  #do not save precomputed sampling coordinates when we needs to be memory efficient
                                                    use_reentrant=False) 
            else:
                ...
                # Interpolate on global grid, #use precomputed sampling coordinates because every mapping is the same
                global_vol_intensity += self.mapLocalToGlobal(local_vol_intensity, global_coord_vec, use_precomputed_sampling_coord=True)
