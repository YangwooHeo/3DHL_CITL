...
if __name__ == '__main__':
    '''Testing new voxelization method'''
    vox = Voxelizer()
    # vox.addMeshes({r"G:\Shared drives\taylorlab\CAL Projects\3DHL\Codebase\3d-holographic-lithography-py\inputs\3d_cross.stl":'print_body',})
    # vox.addMeshes({r"G:\Shared drives\taylorlab\CAL Projects\3DHL\Codebase\3d-holographic-lithography-py\inputs\loft_cross.stl":'print_body',})
    vox.addMeshes({r"G:\Shared drives\taylorlab\CAL Projects\3DHL\Codebase\3d-holographic-lithography-py\inputs\thinker.stl":'print_body',})
    ...
    voxels = vox.voxelize('print_body',layer_thickness=0.5,voxel_value=1,voxel_dtype=np.uint8,square_xy=True,slice_save_path=None)

    print(f'Voxelized body has shape {voxels.shape}')

    import vedo
    import vedo.applications
    vol = vedo.Volume(voxels,mode=0)
